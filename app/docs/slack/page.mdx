export const metadata = {
  ...
  description:
    "A complete walkthrough for wiring an Eve agent to Slack: the bot mental model, Studio's guided setup, the gotchas that cost real debugging time, and how to verify it works.",
};
...
In Studio, go to **Integrations → Channels → the Slack card → Set up**. It's a six-step wizard, starting with a **How it works** explainer that covers the mental model above. (If you'd rather drive it from the registry, `eve add channel/slack` installs the same channel and runs Eve's own setup flow.)

### 1. Bot

Create a new Vercel Connect Slack connector (**Create**, which runs `vercel connect create slack --triggers`), or pick an existing one if you've already got a bot you want to reuse. Then **Open to authorize**: this installs the Eve app into your Slack workspace so the bot actually exists there. The link works once; if it errors, hit Create again for a fresh one.
...
Save your Slack member id (`U…`) or a channel id (`C…`) to an environment variable so proactive and scheduled messages have somewhere to go. Set it on **all environments**: **Production** for the deployed cron, and **Development** so the Schedules tab's local **Test** button can deliver.
...
And a scheduled DM that sends *proactively*. The handler reads the target from an env var and hands the prompt to the Slack channel with `to(...)`:
...
export default defineSchedule({
  ...
  async run({ to, waitUntil, appAuth }) {
    ...
    if (!channelId) return;
    waitUntil(to(slack, { channelId }).send("Remind me to stretch.", { auth: appAuth }));
  },
});
```
...
- **The Channels tab shows the real wiring.** It tells you which **bot** (`bot slack/<uid>`) a channel uses, and shows a **connected to this agent** badge: green when the connector is actually attached to this agent's project, versus **not attached. Finish setup** when it isn't (with a **Finish setup** button). Trust the badge over your memory of what you set up.
- **There's a Remove channel icon** on each row to delete the file. The connector stays attached in Vercel; remove it there if you no longer want it.
